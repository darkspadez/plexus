...
/**
 * Display-only currency settings pulled from CurrencyContext.
 * Costs are always stored and aggregated in USD; `rate` and `symbol` are applied
 * at render time so the underlying data is never mutated.
 */
...
/**
 * Build the list of metrics that can be toggled on/off in the chart.
 ...
 *
 * The cost metric is currency-aware: its label carries the active symbol and its
 * formatter expects values that have already been converted to the display
 * currency (see `chartData`), so it divides by `rate` before re-applying it.
 */
 ...
 *
 * @param data            - Aggregated time-series data points (costs already converted for display)
 * @param chartType       - Which chart visualization to render
 * @param selectedMetrics - Array of metric keys currently toggled on
 * @param metrics         - Currency-aware metric configs from `createMetrics`
...
const renderTimeSeriesChart = (
  ...
  // When cost is the *only* series on the right axis, format that axis as currency
  // and label it, so the converted values are unambiguous.
 ...
 *
 * @param data      - Categorically aggregated data points (costs already converted for display)
...
export const DetailedUsage: React.FC<DetailedUsageProps> = ({
  ...
  const aggregatedData = useMemo(() => {
    if (groupBy === 'time') {
      ...
      // The summary endpoint does not return cost, so derive per-bucket cost
      // from the raw records using the same bucketing the summary uses.
  ...
  /**
   * Currency-aware metric configs, plus a copy of the aggregated data with costs
   * converted into the active display currency. Only the chart consumes
   * `chartData`; tables format raw USD values through `formatCostIn` directly.
   */
  ...
  const rawLogColumns = useMemo<ColumnDef<UsageRecord>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="tabular-nums text-xs text-foreground">
            {formatCostIn(row.original.costTotal || 0, { currency, rate, symbol, decimals: 4 })}
          </span>
        ),
      },
      {
  ...
  const breakdownColumns = useMemo<ColumnDef<AggregatedPoint>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="tabular-nums text-foreground">
            {formatCostIn(row.original.cost, { currency, rate, symbol, decimals: 6 })}
          </span>
        ),
      },
      {
