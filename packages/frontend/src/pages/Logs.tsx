...
// re-renders the whole logs table.
...
// Hoisted to module scope: these are constants, and rebuilding the Set/record on
// every render would both churn allocations and make them unstable closures for
// the memoized cells below.
...
const LogDateCell = React.memo(({ log }: { log: UsageRecord }) => {
...
const LogApiCell = React.memo(({ log }: { log: UsageRecord }) => (
  <div
...
const LogModelCell = React.memo(({ log }: { log: UsageRecord }) => (
    ...
    {log.isVisionFallthrough && log.visionFallthroughModel && (
      ...
      >
        ...
        <button
          ...
          title={isClipboardAvailable() ? 'Copy fallthrough model name' : 'Copy requires HTTPS'}
          disabled={!isClipboardAvailable()}
        >
          <Copy size={10} className="text-foreground-muted hover:text-foreground" />
        </button>
...
interface LogMetaCellProps {
  log: UsageRecord;
  onRetryDetails: (log: UsageRecord) => void;
}

const LogMetaCell = React.memo(({ log, onRetryDetails }: LogMetaCellProps) => (
  <div
    title={
...
const LogStatusCell = React.memo(({ log, onError, onDebug }: LogStatusCellProps) => (
...
const LogDeleteButton = React.memo(({ requestId, onDelete, className }: LogDeleteButtonProps) => (
  <button
    onClick={() => onDelete(requestId)}
    className={cn(
      'bg-transparent border-0 text-foreground-subtle p-1 rounded cursor-pointer transition-all duration-200 flex items-center justify-center hover:bg-danger-subtle hover:text-danger',
      className
    )}
    title="Delete log"
  >
...
// Module-level literal so the memoized delete button keeps stable props.
const DELETE_BUTTON_ROW_HOVER_CLASS = 'opacity-0 group-hover:opacity-100';
...
export const Logs = () => {
  ...
  useEffect(() => {
    ...
    // flight — otherwise the timer would re-render the table for no reason.
  ...
  const handleRetryDetails = useCallback((log: UsageRecord) => {
  ...
  const logsColumns = useMemo<ColumnDef<UsageRecord>[]>(
    () => [
      {
        ...
        cell: ({ row }) => <LogDateCell log={row.original} />,
      },
      {
        id: 'apiKey',
      ...
      },
      {
        ...
        cell: ({ row }) => <LogApiCell log={row.original} />,
      },
      {
        id: 'model',
        ...
        cell: ({ row }) => <LogModelCell log={row.original} />,
      },
      {
        id: 'tokens',
        header: 'Tokens',
      ...
      },
      {
        ...
        cell: ({ row }) => <LogMetaCell log={row.original} onRetryDetails={handleRetryDetails} />,
      },
      {
        id: 'status',
        header: 'Status',
        ...
        cell: ({ row }) => (
          <LogStatusCell log={row.original} onError={handleError} onDebug={handleDebug} />
        ),
      },
      {
        id: 'delete',
        ...
        cell: ({ row }) => (
          <LogDeleteButton
            ...
            className={DELETE_BUTTON_ROW_HOVER_CLASS}
          />
        ),
      },
    ],
    ...
    [sortBy, sortDir, handleError, handleDebug, handleRetryDetails, handleDelete]
  );

  const renderMobileActions = useCallback(
    (log: UsageRecord) => <LogDeleteButton requestId={log.requestId} onDelete={handleDelete} />,
    [handleDelete]
  );
