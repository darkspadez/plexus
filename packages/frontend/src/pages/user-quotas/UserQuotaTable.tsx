...
import type { FormatCostInOptions } from '../../lib/format';
...
const formatLimit = (q: UserQuota, cost: FormatCostInOptions): string => {
  switch (q.limitType) {
    case 'cost':
      return formatCostIn(q.limit, cost);
    case 'tokens':
...
export const UserQuotaTable: React.FC<Props> = ({ rows, onEdit, onDelete, keysUsingCounts }) => {
  ...
  const columns: ColumnDef<UserQuotaRow>[] = React.useMemo(() => {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-mono text-xs tabular-nums text-foreground">
            {formatLimit(row.original.quota, { currency, rate, symbol })}
          </span>
        ),
      },
      {
    ...
    return cols;
  }, [onEdit, onDelete, keysUsingCounts, currency, rate, symbol]);
