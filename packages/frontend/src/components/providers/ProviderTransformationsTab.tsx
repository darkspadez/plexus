...
export function ProviderTransformationsTab({ f }: { f: ProviderFormApi }) {
  ...
  // The backend replays adapter entries in order, so a LATER entry overrides an
  // earlier one — read the last match, not the first (resolveAdapters, adapter-resolver.ts).
  let toolIdEntry: any;
  for (const candidate of adapterEntriesForCount) {
    const name = typeof candidate === 'string' ? candidate : candidate?.name;
    if (name === ANTHROPIC_TOOL_ID_ADAPTER) toolIdEntry = candidate;
  }
  const toolIdMode: 'auto' | 'on' | 'off' = !toolIdEntry
    ? 'auto'
    : typeof toolIdEntry === 'string' || toolIdEntry.enabled !== false
      ? 'on'
      : 'off';

  const setToolIdMode = (value: 'auto' | 'on' | 'off') => {
    const withoutEntry = adapterEntriesForCount.filter(
      (e: any) => (typeof e === 'string' ? e : e?.name) !== ANTHROPIC_TOOL_ID_ADAPTER
    );
    const next =
      value === 'auto'
        ? withoutEntry
        : [
            ...withoutEntry,
            { name: ANTHROPIC_TOOL_ID_ADAPTER, options: {}, enabled: value === 'on' },
          ];
    setEditingProvider({ ...editingProvider, adapter: next });
  };

  // Advisory mirror of the backend gate (adapter-resolver.ts): it only fires on the
  // Messages wire format, so a record base URL counts only where the Messages dispatch
  // would go — a chat-only anthropic.com entry never receives one. A string base URL
  // containing anthropic.com IS a Messages provider by inference (getProviderTypes).
  ...
  const toolIdBaseUrls = editingProvider.apiBaseUrl;
  const toolIdAutoDetected =
    editingProvider.useClaudeMasking === true ||
    (typeof toolIdBaseUrls === 'string'
      ? isAnthropicUrl(toolIdBaseUrls)
      : Object.entries(toolIdBaseUrls ?? {}).some(([key, value]) => {
          ...
          <div className="flex items-center justify-between gap-3 py-2">
            <div className="min-w-0 flex-1">
              ...
              <div
                ...
                title={
                  ...
                  "targets on this provider's Messages endpoint: base URL containing anthropic.com, " +
                  'Anthropic OAuth, or Claude masking. Choose Enabled to force it for ' +
            ...
            <div className="w-[210px] shrink-0">
              <Select<'auto' | 'on' | 'off'>
                ...
                value={toolIdMode}
                onChange={setToolIdMode}
                options={[
                  {
                    value: 'auto',
                    label: `Auto (currently: ${toolIdAutoDetected ? 'enabled' : 'disabled'})`,
                  },
