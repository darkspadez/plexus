...
export function ProviderModelsEditor({
  ...
  // Probe / copy / remove cluster — shared by list rows and the detail-view header.
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            {renderModelActions(selectedModelId, models[selectedModelId])}
          </div>
        </div>
        {Object.entries(models)
          .filter(([id]) => id === selectedModelId)
          .map(([mId, mCfg]: [string, any]) => {
            const testKey = `${editingProvider.id}-${mId}`;
            const testState = testStates[testKey];
            ...
            return (
              ...
              >
                ...
                <SectionCard
                  ...
                  onOpenChange={(open) =>
                    setModelAdaptersOpen((prev) => ({ ...prev, [mId]: open }))
                  }
                          ...
                          >
                            ...
                            <div>
                              ...
                              <div className="font-sans text-[11px] leading-snug text-foreground-muted">
                                Enabled by default. Removes generation options GPT-5 does not
                                accept.
                              </div>
                            </div>
                          </label>
                      ...
                      return (
                        <div className="mt-1 border-t border-border pt-1.5 sm:col-span-2">
                          ...
                          <div className="font-sans text-[10px] leading-snug text-foreground-subtle mb-2">
                            When ANY condition matches, rewrite the model name. Use dotted paths
                            like reasoning.enabled.
                          </div>
                ...
                <SectionCard
                  ...
                  onOpenChange={(open) =>
                    setModelAdvancedOpen((prev) => ({ ...prev, [mId]: open }))
                  }
                  bodyClassName="bg-surface-sunken p-2"
                  extra={
                    <>
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {Object.entries(models).map(([mId, mCfg]: [string, any]) => (
            <div
              key={mId}
            ...
            >
              ...
              <div className="flex flex-shrink-0 items-center gap-2 pr-2">
                {renderModelActions(mId, mCfg, { errorBadge: true })}
              </div>
            </div>
          ))}
        </div>
