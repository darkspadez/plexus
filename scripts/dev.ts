...
import { existsSync, writeFileSync, unlinkSync, rmSync, statSync, readFileSync } from 'fs';
...
// A saved backup newer than our last restore also wins, even when the dev DB already exists
// (e.g. after `bun run prep-dev:save`). Deliberately not folded into isFreshDb(), which gates
// the synthetic seeder and must stay a pure "is there a database file?" check.
const restoreWinsThisBoot =
  fullMode && backupSource && (isFresh || savedBackupIsNewerThanLastRestore());
...
if (markerPresent) {
  ...
  if (await isPortInUseAnyFamily(mockPort)) {
    ...
    spawnManaged('bun', ['run', 'scripts/mock-upstream.ts'], process.cwd());
  }
}
...
(async () => {
  ...
  spawnManaged('bun', ['run', 'scripts/live-ticker.ts'], process.cwd());
})();
